(ns marketentry.marketentryllm
  ...
  the RUS public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-ru-entity?=" (:has-ru-entity? e)
                        " inn-verified?=" (:inn-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-ru-entity? e))
                              (:has-ru-entity? e))
                          (or (not (:requires-inn? e))
                              (:inn-verified? e)))
                   0.9 0.3)}))
