(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a Russia-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Russia-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-ru-entity?
                                       true` (near-universal for Russia
                                       ...
                                       Grounded in Russian legal entity
                                       (INN/OGRN) typically required for EIS
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(INN/統一資格/EIS/zakupki.gov.ru登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- ru-entity-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-ru-entity? true`, INDEPENDENTLY verify
  `:has-ru-entity?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-ru-entity?` ground truth (most Russia public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-ru-entity? e))
                 (not (true? (:has-ru-entity? e))))
        [{:rule :ru-entity-missing
          :detail (str subject " はロシア居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- inn-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-inn? true`, INDEPENDENTLY check
  `:inn-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-inn? e))
                 (not (true? (:inn-verified? e))))
        [{:rule :inn-unverified
          :detail (str subject " はINN確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (ru-entity-missing-violations request st)
                           (engagement-fee-mismatch-violations request st)
                           (inn-unverified-violations request st)
